...
function hgiNetwork() {
  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      if (linkDistance == 0) {
        linkDistance = Math.min((height) / 4, (width) / 4);
      }
      ...
      data.nodes.forEach(function (node) {
        (node.weight > centerNode.weight) ? centerNode = node : 0;
      });
      ...
      if (doForceBasedLabels) {
        ...
        data.labelNodes = [];
        data.nodes.forEach(function (node) {
          data.labelNodes.push({node: node});
          data.labelNodes.push({node: node});
        });
      ...
      if (!noLinks) {
        ...
        var gLinks = gLinks
          .data(data.links.filter(function (d) {
            if (!isDirectedGraph || d.doubly == false) return d;
          }))
        ...
        if (addCoefficientMarkers) {
          ...
          gLinks.append("text")
            ...
            .text(function (d) {
              (d.coef > 0) ? d.coefSign = "+" : d.coefSign = "-";
              return d.coefSign;
            });//return d.coef; });
        ...
        gLinks.append("line")
          .style("stroke-width", function (d) {
            d.width = (minStrokeWidth + Math.min(maxStrokeWidth, maxStrokeWidth * Math.max(0, (Math.abs(d.coef) - 0.2) * 2)));
            return d.width + "px";
          });
      ...
      if (!addGradients) {
        gNodes.append("circle")
          ...
          .attr("r", function (d) {
            return d.radius;
          }) // d.radius=(10+10*(d.weight/centerNode.weight)); return d.radius; })
      ...
      if (allowDragging) {
        ...
        netLayout.drag().on("dragstart", function (d) {
          d3.select(this).classed("fixed", d.fixed = true);
        });

        if (doForceBasedLabels) {
          netLayout.drag().on("drag", function () {
            labelLayout.resume();
          });
        }
      ...
      function tickLayout() {
        ...
        gLinks.selectAll('line').attr("x1", function (d) {
            return d.source.x;
          })
          .attr("y1", function (d) {
            return d.source.y;
          ...
          if (DEBUG) { // Draws circles at point where coefficient markers (+ or -) should be drawn
            gLinks.selectAll('circle').attr("transform", function (d) {
              ...
              if (!isDirectedGraph && (d.source.weight < d.target.weight)) {
                targetRadius = d.source.radius;
                targetX = d.source.x;
                targetY = d.source.y;
                fromX = d.target.x;
                fromY = d.target.y;
              } else {
                targetRadius = d.target.radius;
                targetX = d.target.x;
                targetY = d.target.y;
                fromX = d.source.x;
                fromY = d.source.y;
      ...
      function tickLabel() {
        ...
        // Update node and link positions
        gLabelLinks.attr("x1", function (d) {
            return d.source.x;
          })
          .attr("y1", function (d) {
            return d.source.y;
        ...
        gLabelNodes.attr("transform", function (d) {
          return "translate(" + d.x + "," + d.y + ")";
        });
  ...
  chart.doStaticLayout = function (_) {
    if (!arguments.length) {
      return doStaticLayout;
    }
    doStaticLayout = _;
    return chart;
  };
  ...
  chart.minStrokeWidth = function (_) {
    if (!arguments.length) {
  ...
  chart.maxStrokeWidth = function (_) {
    if (!arguments.length) {
  ...
  chart.allowDragging = function (_) {
    if (!arguments.length) {
  ...
  chart.doForceBasedLabels = function (_) {
    if (!arguments.length) {
      return doForceBasedLabels;
    }
    doForceBasedLabels = _;
    return chart;
  };
  chart.addCoefficientMarkers = function (_) {
    if (!arguments.length) {
      return addCoefficientMarkers;
    }
    addCoefficientMarkers = _;
    return chart;
  };
  chart.coefMarkerDistance = function (_) {
    if (!arguments.length) {
      return coefMarkerDistance;
    }
    coefMarkerDistance = _;
    return chart;
  };
  chart.coefMarkerOffset = function (_) {
    if (!arguments.length) {
      return coefMarkerOffset;
    }
    coefMarkerOffset = _;
    return chart;
  };
  chart.isDirectedGraph = function (_) {
    if (!arguments.length) {
  ...
  chart.doWeightedCharge = function (_) {
    if (!arguments.length) {
      return doWeightedCharge;
    }
    doWeightedCharge = _;
    return chart;
  };
...
function hgiPie() {
  ...
  chart.offsetLegends = function (_) {
    if (!arguments.length) {
      return offsetLegends;
    }
    offsetLegends = _;
    return chart;
  };
...
function hgiScatter() {
  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      // Update the dots
      var gDots = gChart.select(".dots").selectAll(".dot").data(data.filter(function (d) {
        return (isNaN(d[0]) || isNaN(d[1]) || d[0] == null || d[1] == null) ? 0 : 1;
      })).enter();
      ...
      if (addLOESS) {
        ...
        gChart.append('path')
          .datum(function () {
            ...
            var dataNARM = data.filter(function (d) {
              return (isNaN(d[0]) || isNaN(d[1]) || d[0] == null || d[1] == null) ? 0 : 1;
            });

            var xValues = dataNARM.map(function (d) {
              return xScale(d[0]);
            });
            var yValues = dataNARM.map(function (d) {
              return yScale(d[1])
            });
...
function hgiBubble() {
  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      // Update scales
      (xDomain == 0) ? xScale.domain([0, d3.max(data, function (d) {
        return d[0];
      })]) : xScale.domain(xDomain);
      (yDomain == 0) ? yScale.domain(data.map(function (d) {
        return d[1];
      ...
      gBubbles.append("circle")
        ...
        .attr("cy", function (d) {
          return yScale(d[1]) + yScale.rangeBand() / 2
        })
        .attr("r", function (d) {
          return 7 + 4 * d[2]
        });
      ...
      if (addLabels) {
        gBubbles.append("text")
          .attr("class", "label")
          .attr("y", function (d) {
            return yScale(d[1]) + yScale.rangeBand() / 2 + 4
          })
          .attr("x", function (d) {
            return xScale(d[0]) - (7 + 4 * d[2]) - 4
          })
  ...
  chart.weightMapping = function (_) {
    if (!arguments.length) {
...
function hgiBar() {
  ...
  var xValue = function (d) {
    ...
    errorValue = function (d) {
      return d[errorMapping];
    },
    freqValue = function (d) {
      return d[freqMapping];
    };
  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      // Create SVG with data appended
      var svg = d3.select(this).selectAll("svg").data([data.filter(function (d) {
        return (isNaN(d[1]) || d[1] == 0) ? 0 : 1
      })]);
      var gChart = svg.enter().append("svg").append("g");
      ...
      // Update the bars
      var gBars = gChart.select(".bars").selectAll(".bar").data(data.filter(function (d) {
        return (isNaN(d[1]) || d[1] == 0) ? 0 : 1
      })).enter();
      ...
      if (addLabels) {
        gBars.append("text")
          ...
          .attr("dy", ".75em")
          .attr("y", function (d) {
            return yScale(d[0]) - 5 + yScale.rangeBand() / 2;
          })
          .attr("x", function (d) {
            return xScale(d[1]) - 5;
          })
      ...
      if (addFrequencies) {
        ...
        gBars.append("text")
          ...
          .text(function (d) {
            if (xScale(d[1]) > 27) {
              return formatCount(d[3]) + "x";
            }
          }); // HACK - Do not display text when bar is smaller than 27px
      }
      ...
      if (addError) {
        ...
        gBars.insert("line")
          ...
          .attr("y1", function (d) {
            return yScale(d[0]) + yScale.rangeBand() / 2;
          })
          .attr("x2", function (d) {
            return xScale(d[1] + d[2]);
          })
          .attr("y2", function (d) {
            return yScale(d[0]) + yScale.rangeBand() / 2;
          })
        ...
        gBars.append("line")
          ...
          .attr("y1", function (d) {
            return yScale(d[0]) + yScale.rangeBand() / 4
          })
          .attr("x2", function (d) {
            return xScale(d[1] + d[2])
          })
          .attr("y2", function (d) {
            return yScale(d[0]) + yScale.rangeBand() / 1.33;
          })
  ...
  chart.addFrequencies = function (_) {
    if (!arguments.length) {
...
function hgiHistogram() {
  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      // Update scales
      (xDomain == 0) ? xScale.domain(data.map(function (d) {
        return d[0];
      })) : xScale.domain(xDomain);
      (yDomain == 0) ? yScale.domain([0, d3.max(data, function (d) {
        return d[1];
      })]) : yScale.domain(yDomain);
      ...
      if (addLabels) {
        gBars.append("text")
          ...
          .text(function (d) {
            if ((height - margin.top - margin.bottom - yScale(d[1])) > 22) {
              return formatCount(d[2]);
            }
          }); // HACK - Do not display text when bar is smaller than 22px
      }
      ...
      if (addError) {
        ...
        gBars.append("line")
          .attr("class", "errorbar")
          .attr("x1", function (d) {
            return xScale(d[0]) + xScale.rangeBand() / 4
          })
          .attr("y1", function (d) {
            return yScale(d[1] + d[2])
          })
          .attr("x2", function (d) {
            return xScale(d[0]) + xScale.rangeBand() / 1.33;
          })
          .attr("y2", function (d) {
            return yScale(d[1] + d[2])
          })
...
function hgiPairedHistogram() {
  ...
  var xValue = function (d) {
    ...
    error1Value = function (d) {
      return d[errorMapping[0]];
    },
    error2Value = function (d) {
      return d[errorMapping[1]];
    };
    ...
    selection.each(function (data) {
      ...
      if (addError) {
        ...
        gBars.append("line")
          .attr("class", "errorbar")
          .attr("x1", function (d) {
            return xScale(d[0]) + xScale.rangeBand() / 8
          })
          .attr("y1", function (d) {
            return yScale(d[1] + d[3])
          })
          .attr("x2", function (d) {
            return xScale(d[0]) + xScale.rangeBand() / 2.67;
          })
          .attr("y2", function (d) {
            return yScale(d[1] + d[3])
          });
      ...
      gBars.append("rect")
        ...
        .attr("height", function (d) {
          return height - margin.top - margin.bottom - yScale(d[2]);
        });
      ...
      if (addError) {
        ...
        gBars.append("line")
          .attr("class", "errorbar")
          .attr("x1", function (d) {
            return xScale(d[0]) + xScale.rangeBand() / 1.6
          })
          .attr("y1", function (d) {
            return yScale(d[2] + d[4])
          })
          .attr("x2", function (d) {
            return xScale(d[0]) + xScale.rangeBand() / 1.14;
          })
          .attr("y2", function (d) {
            return yScale(d[2] + d[4])
          });
      ...
      if (addLabels) {
        gBars.append("text")
          ...
          .text(function (d) {
            if ((height - margin.top - margin.bottom - yScale(d[2])) > 22) {
              return formatCount(d[2]);
            }
          }); // HACK - Do not display text when bar is smaller than 22px
...
function hgiLine() {
  ...
  function chart(selection) {
    selection.each(function (data) {
      ...
      if (addVerticalIndicator) {
        ...
        var indXBg = gChart.append("rect").attr("class", "verticalLineBGLabel").attr({
          width: 30,
          height: 15,
          y: (height - margin.top - margin.bottom + 7)
        }).style("display", "none");
        ...
        rect.on("mouseover", function () {
          ...
          indX.style("display", null);
          indXBg.style("display", null)
        })
        rect.on("mouseout", function () {
          verticalLine.style("display", "none");
          indX.style("display", "none");
          indXBg.style("display", "none");
          gChart.selectAll('circle').classed("hover", 0)
        })
        rect.on('mousemove', function () {
          ...
          gChart.selectAll('circle').classed("hover", function (data) {
            return (data[0] == d[0])
          }); // CSS based transition

          // Update text indicator and show on x axis
          indX.text(d[0]).attr("transform", function () {
            return "translate(" + xScale(d[0]) + ",0)"
          });
          indXBg.attr("transform", function () {
            return "translate(" + (xScale(d[0]) - 15) + ",0)"
          });
  ...
  chart.addVerticalIndicator = function (_) {
    if (!arguments.length) {
      return addVerticalIndicator;
    }
    addVerticalIndicator = _;
    return chart;
  };
  chart.overlayOffset = function (_) {
    if (!arguments.length) {
