  ...
  describe("with json parser", function() {
    ...
    describe('getYDataFromJson', function() {
      ...
      beforeEach(function () {
        ...
        result = json_parser.getYDataFromJson(node_names);
      });

      it('have the correct dimensions', function() {
        var current_result = dimensions(result);
        var number_of_variables = node_names.length;
        expect(current_result).toEqual([89, number_of_variables]);
      });
    });

    describe('getExogenVariables', function () {
      var result,
      node_names;
      beforeEach(function () {
        node_names = json_parser.nodeKeysFromJson();
        result = json_parser.getExogenVariables(node_names);
      });

      it('have the correct dimensions', function() {
        var total_number_of_vars = 29;
        var current_result = dimensions(result);
        var number_of_exo_variables = total_number_of_vars - node_names.length;
        expect(current_result).toEqual([89, number_of_exo_variables]);
      });
    });
  });
});
